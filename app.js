...
function renderMovies() {
  let filtered = data.filter(m => { const q=state.query.toLowerCase(); return (state.category==='All'||m.category===state.category) && (!q||`${m.title} ${m.originalTitle||''} ${m.category} ${(m.genres||[]).join(' ')} ${m.year||''} ${m.director||''}`.toLowerCase().includes(q)); });
  ...
  grid.innerHTML=filtered.map((movie,i)=>{ const cover=movie.cover&&!movie.cover.includes('PASTE-')?`<img loading="lazy" src="${escapeHtml(movie.cover)}" alt="${escapeHtml(movie.title)} cover" onerror="this.style.display='none'">`:`<div class="fallback">${escapeHtml(movie.title)}</div>`; return `<article class="movie-card" data-index="${data.indexOf(movie)}" tabindex="0" role="button">${cover}<span class="movie-badge">${escapeHtml(movie.category||'Movie')}</span><div class="movie-info"><h3>${escapeHtml(movie.title)}</h3><p>${movie.year||'—'} · ${movie.status==='watched'?'Watched':'To watch'}</p></div></article>`; }).join('');
...
function renderCategoryCards(){const real=allCategories.filter(c=>c!=='All');categoryCards.innerHTML=real.map(c=>{const n=data.filter(m=>m.category===c).length;return `<button class="category-card" data-category="${escapeHtml(c)}">${escapeHtml(c)}<small>${n} ${n===1?'movie':'movies'}</small></button>`}).join('');categoryCards.querySelectorAll('button').forEach(b=>b.onclick=()=>{state.category=b.dataset.category;document.getElementById('library').scrollIntoView({behavior:'smooth'});renderTabs();renderMovies();});}
function renderStats(){document.getElementById('movieCount').textContent=data.length;document.getElementById('categoryCount').textContent=new Set(data.map(m=>m.category).filter(Boolean)).size;document.getElementById('watchCount').textContent=data.filter(m=>m.status!=='watched').length;}

function setText(id,value){document.getElementById(id).textContent=value||'—';}
function openMovie(index){
  const m=data[index]; if(!m)return;
  setText('detailTitle',m.title); setText('detailCategory',m.category||'Movie'); setText('detailMeta',[m.year,m.runtime,m.ageRating,m.rating,m.status==='watched'?'Watched':'To watch'].filter(Boolean).join(' · ')); setText('detailDescription',m.description);
  ...
  const facts=[['Original title',m.originalTitle],['Release date',m.releaseDate],['Format',m.format],['Storage / category',m.category],['Location',m.location],['Director',m.director],['Studio',m.studio],['Language',m.language],['Country',m.country],['Franchise',m.franchise],['Collection',m.collection]];
